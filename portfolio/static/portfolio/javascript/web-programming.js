window.addEventListener("load", () => {
  fetch(
    ...
    .then((data) => {
      let meteorologyEl = document.getElementById("meteorology");

      for (const day of data.data) {
        let dayDivEl = document.createElement("div");
        dayDivEl.classList.add("meteorology-day");
  ...
  fetch("https://api.github.com/users/JMatos1221/repos")
    .then((response) => response.json())
    .then((data) => {
      ...
      for (const repo of data) {
        ...
        githubRepoEl.classList.add("github-repo");
